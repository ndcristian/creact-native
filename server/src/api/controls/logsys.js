...
var infoStream = fs.createWriteStream('logs/info-'+new Date().toISOString()+'.txt');
...
// file if needed.
var errorStream = fs.createWriteStream('logs/error-'+new Date().toISOString()+'.txt');
...
var debugStream = fs.createWriteStream('logs/debug-'+new Date().toISOString()+'.txt');
...
Logger.info = function(msg) {
  ...
  infoStream.write(JSON.stringify({date:new Date().toISOString(), info:msg},null,2));
};
